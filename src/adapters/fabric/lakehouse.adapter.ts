...
export class FabricLakehouseAdapter {
	...
	async fullSync(
		...
		const filter = tenantId ? `WHERE c.tenantId = '${tenantId}'` : "";
		...
		if (sessions.length > 0) {
			const r = await this.writeRows(
				"sessions",
				sessions.map((s) => this.flattenSession(s)),
				partition,
			);
		...
		if (observations.length > 0) {
			const r = await this.writeRows(
				"observations",
				observations.map((o) => this.flattenObservation(o)),
				partition,
			);
		...
		if (memories.length > 0) {
			const r = await this.writeRows(
				"memories",
				memories.map((m) => this.flattenMemory(m)),
				partition,
			);
		...
		if (graphNodes.length > 0) {
			const r = await this.writeRows(
				"graph_nodes",
				graphNodes.map((n) => this.flattenGraphNode(n)),
				partition,
			);
		...
		if (graphEdges.length > 0) {
			const r = await this.writeRows(
				"graph_edges",
				graphEdges.map((e) => this.flattenGraphEdge(e)),
				partition,
			);
	...
	async healthCheck(): Promise<{ status: string; workspace?: string }> {
		...
		try {
			...
			const iter = fsClient
				.listPaths({ path: this.getLakehousePath() })
