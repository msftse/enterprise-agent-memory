...
function createMockCtx(overrides?: Partial<{
  ...
  return {
    cosmos: {
      ...
      read: vi.fn().mockImplementation(async (_container: string, id: string) => {
        return readResults[id] ?? null;
      }),
      update: vi.fn().mockResolvedValue(undefined),
      query: vi.fn().mockImplementation(async (_container: string, opts: any) => {
...
describe('createGraphNode', () => {
  it('creates a node with generated id and tenant', async () => {
    ...
    const node = await createGraphNode('t1', {
      type: 'file',
      name: '/src/main.ts',
      properties: {},
...
describe('createGraphEdge', () => {
  it('creates an edge with generated id and tenant', async () => {
    const ctx = createMockCtx();
    const edge = await createGraphEdge('t1', {
      type: 'uses',
      sourceNodeId: 'node-1',
      targetNodeId: 'node-2',
      weight: 1.0,
  ...
  it('creates nodes and edges from LLM extraction', async () => {
    const ctx = createMockCtx({
      extractResult: {
        ...
        edges: [
          { type: 'uses', source: '/src/main.ts', target: 'express' },
        ],
      },
    });
...
describe('traverseGraph', () => {
  it('returns nodes and edges in outbound traversal', async () => {
    const node1 = {
      id: 'n1', tenantId: 't1', type: 'file', name: 'main.ts',
      properties: {}, sourceObservationIds: [], createdAt: '2024-01-01',
    };
    const node2 = {
      id: 'n2', tenantId: 't1', type: 'library', name: 'express',
      properties: {}, sourceObservationIds: [], createdAt: '2024-01-01',
    };
    const edge = {
      id: 'e1', tenantId: 't1', type: 'uses',
      sourceNodeId: 'n1', targetNodeId: 'n2', weight: 1,
      sourceObservationIds: [], createdAt: '2024-01-01',
    };
    ...
    (ctx.cosmos.query as any).mockImplementation(async (_: string, opts: any) => {
    ...
    const result = await traverseGraph('t1', 'n1', { direction: 'outbound', maxDepth: 2 }, ctx);
  ...
  it('respects maxDepth limit', async () => {
    const node1 = {
      id: 'n1', tenantId: 't1', type: 'file', name: 'a.ts',
      properties: {}, sourceObservationIds: [], createdAt: '2024-01-01',
    };
